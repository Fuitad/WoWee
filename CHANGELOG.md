...
- **Tilted doodads sat wrong.** ADT stores doodad and building placements in different chunks with the same rotation field, and this client builds the same three numbers from both — then composed them in opposite orders. The building path had the derivation written in its own comment and the buildings on screen agreed with it; the doodad path applied the identical triple backwards
- Three earlier attempts permuted the axes and were reverted. The reason none of them could be judged: with no pitch and no roll the two orders are *the same rotation*, and nearly every doodad in the world stands upright — so any permutation looks right against a tree, and only a genuinely tilted placement can tell them apart. The test now pins that too, so the upright case cannot be offered as evidence again
